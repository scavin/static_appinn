...
jQuery(document).ready(function(a){a(".wpfp-link").live("click",function(){dhis=a(this);wpfp_do_js(dhis,1);if(dhis.hasClass("remove-parent")){dhis.parent("li").fadeOut()}return false})});function wpfp_do_js(b,a){loadingImg=b.prev();loadingImg.show();beforeImg=b.prev().prev();beforeImg.hide();url=document.location.href.split("#")[0];params=b.attr("href").replace("?","")+"&ajax=1";if(a){jQuery.get(url,params,function(c){b.parent().html(c);if(typeof wpfp_after_ajax=="function"){wpfp_after_ajax(b)}loadingImg.hide()})}};
